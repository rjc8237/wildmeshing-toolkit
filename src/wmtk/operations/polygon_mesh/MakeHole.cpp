...
MakeHole::MakeHole(Mesh& m, const Tuple& f, const OperationSettings<MakeHole>& settings)
    : AtomicOperation(m)
    , m_settings(settings)
    , m_tuple(f)
{}

std::string MakeHole::name() const
{
    return "polygon_mesh_make_hole";
}

bool MakeHole::execute()
{
    assert(precondition());

    make_hole(get_face(get_halfedge_from_tuple(m_tuple)));
...
bool MakeHole::precondition() const
{
    ...
    // Nothing to check if the face is already a hole
