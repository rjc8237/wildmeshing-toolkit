...
TEST_CASE("polygon_compound_atomic_operations", "[operations][polygon]")
{
    ...
    polygon_mesh::Splice op2(m, h, g, op_settings);
    success = op2();
    CHECK(success);
    CHECK(m.is_valid_connectivity());
    ...
    polygon_mesh::Splice op3(m, h, g, op_settings);
    success = op3();
    CHECK(success);
    CHECK(m.is_valid_connectivity());
    CHECK(m.euler_characteristic() == 4);
    ...
    polygon_mesh::DeleteBubble op4(m, h, op_settings);
    success = op4();
    CHECK(success);
    CHECK(m.is_valid_connectivity());
    CHECK(m.euler_characteristic() == 2);
